...
from .layers_vq import ( VectorQuantizer, EncodeResidualStack, DecodeResidualStack, DecodeResidualAdaINStack, Jitter)
...
class Model(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            # z_conti_loss = (z_vq[:,:,1:] - z_vq[:,:,:-1]).pow(2).sum() / mean_factor

            # z_cyc_loss = (zhat - z_vq).pow(2).sum() / mean_factor
            # z_cyc_loss += (zhat2 - z_vq).pow(2).sum() / mean_factor
            
            x_loss = log_loss(xhat, x) / mean_factor

            loss = x_loss + z_qut_loss + self.beta * z_enc_loss # + z_conti_loss
            
            losses = {'Total': loss.item(),
                      'VQ loss': z_enc_loss.item(),
                      'Entropy': entropy.item(),
                      # 'Z conti.': z_conti_loss.item(),
